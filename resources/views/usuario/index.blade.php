...
<div class="container mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div class="d-flex gap-2">
            <a href="{{ route('usuario.create') }}" class="btn btn-success">➕ Agregar Usuario</a>
        ...
        <div class="search-container">
            <form method="GET" action="{{ route('usuario.index') }}" class="search-form">
                ...
                @if(request('search'))
                    <a href="{{ route('usuario.index') }}" class="btn-clear">Limpiar</a>
                @endif
            </form>
        </div>
    </div>

    <!-- PAGINACIÓN ARRIBA (MANTIENE EL ESTILO ORIGINAL) -->
    <div class="pagination-minimal">
        @if ($usuarios->onFirstPage())
            <span class="pagination-arrow disabled">❮</span>
        @else
            <a href="{{ $usuarios->previousPageUrl() }}{{ request('search') ? '&search=' . request('search') : '' }}" class="pagination-arrow">❮</a>
        @endif
        
        @if ($usuarios->hasMorePages())
            <a href="{{ $usuarios->nextPageUrl() }}{{ request('search') ? '&search=' . request('search') : '' }}" class="pagination-arrow">❯</a>
        @else
            <span class="pagination-arrow disabled">❯</span>
        @endif
    </div>
            ...
            <tr>
                ...
                <td>
                    ...
                    <div class="action-buttons">
                        <a href="{{ route('usuario.edit', $item->id) }}" class="btn btn-warning btn-sm btn-with-icon">
                            ✏️ Editar
                        </a>
                        <form action="{{ route('usuario.destroy', $item->id) }}" method="POST">
                            @csrf @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm btn-with-icon" onclick="return confirm('¿Eliminar usuario?')">
                                🗑️ Eliminar
                            </button>
                        </form>
                    </div>
