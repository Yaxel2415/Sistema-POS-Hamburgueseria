...
<style>
    ...
    thead tr:hover {
        background-color: #000; /* evita hover */
    }
...
<div class="container mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="search-container">
            <form method="GET" action="{{ route('cliente.index') }}" class="search-form">
                ...
                @if(request('search'))
                    <a href="{{ route('cliente.index') }}" class="btn-clear">Limpiar</a>
                @endif
            </form>
        </div>
    </div>
    ...
    <div class="pagination-minimal">
        ...
        @else
            <a href="{{ $clientes->previousPageUrl() }}{{ request('search') ? '&search=' . request('search') : '' }}" class="pagination-arrow">❮</a>
        @endif
        
        @if ($clientes->hasMorePages())
            <a href="{{ $clientes->nextPageUrl() }}{{ request('search') ? '&search=' . request('search') : '' }}" class="pagination-arrow">❯</a>
        @else
            <span class="pagination-arrow disabled">❯</span>
        @endif
    </div>
            ...
            <tr>
                ...
                <td>
                    <div class="action-buttons">
                        <a href="{{ route('cliente.edit', $item->id) }}" class="btn btn-warning btn-sm btn-with-icon">
                            ✏️ Editar
                        </a>
                        <form action="{{ route('cliente.destroy', $item->id) }}" method="POST">
                            @csrf @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm btn-with-icon" onclick="return confirm('¿Eliminar cliente?')">
                                🗑️ Eliminar
                            </button>
                        </form>
                    </div>
